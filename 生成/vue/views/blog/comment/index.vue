<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="文章ID (关联 blog_article.article_id)" prop="articleId">
        <el-input
          v-model="queryParams.articleId"
          placeholder="请输入文章ID (关联 blog_article.article_id)"
          clearable
          @keyup.enter="handleQuery"
        />
      </el-form-item>
      <el-form-item label="评论用户ID (关联 sys_user.user_id, 可为空)" prop="userId">
        <el-input
          v-model="queryParams.userId"
          placeholder="请输入评论用户ID (关联 sys_user.user_id, 可为空)"
          clearable
          @keyup.enter="handleQuery"
        />
      </el-form-item>
      <el-form-item label="评论人昵称 (游客或用户昵称)" prop="nickname">
        <el-input
          v-model="queryParams.nickname"
          placeholder="请输入评论人昵称 (游客或用户昵称)"
          clearable
          @keyup.enter="handleQuery"
        />
      </el-form-item>
      ...
      <el-form-item label="父评论ID (0=对文章的评论, &gt;0=回复某条评论)" prop="parentId">
        <el-input
          v-model="queryParams.parentId"
          placeholder="请输入父评论ID (0=对文章的评论, &gt;0=回复某条评论)"
          clearable
          @keyup.enter="handleQuery"
        />
      </el-form-item>
    ...
    <el-table v-loading="loading" :data="commentList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="文章ID (关联 blog_article.article_id)" align="center" prop="articleId" />
      <el-table-column label="评论用户ID (关联 sys_user.user_id, 可为空)" align="center" prop="userId" />
      <el-table-column label="评论人昵称 (游客或用户昵称)" align="center" prop="nickname" />
      <el-table-column label="评论人邮箱" align="center" prop="email" />
      <el-table-column label="父评论ID (0=对文章的评论, &gt;0=回复某条评论)" align="center" prop="parentId" />
      <el-table-column label="状态 (0=待审核, 1=已显示)" align="center" prop="status" />
    ...
    <el-dialog :title="title" v-model="open" width="500px" append-to-body>
      <el-form ref="commentRef" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="文章ID (关联 blog_article.article_id)" prop="articleId">
          <el-input v-model="form.articleId" placeholder="请输入文章ID (关联 blog_article.article_id)" />
        </el-form-item>
        <el-form-item label="评论用户ID (关联 sys_user.user_id, 可为空)" prop="userId">
          <el-input v-model="form.userId" placeholder="请输入评论用户ID (关联 sys_user.user_id, 可为空)" />
        </el-form-item>
        <el-form-item label="评论人昵称 (游客或用户昵称)" prop="nickname">
          <el-input v-model="form.nickname" placeholder="请输入评论人昵称 (游客或用户昵称)" />
        </el-form-item>
        <el-form-item label="评论人邮箱" prop="email">
        ...
        <el-form-item label="父评论ID (0=对文章的评论, &gt;0=回复某条评论)" prop="parentId">
          <el-input v-model="form.parentId" placeholder="请输入父评论ID (0=对文章的评论, &gt;0=回复某条评论)" />
        </el-form-item>
...
const data = reactive({
  ...
  rules: {
    articleId: [
      { required: true, message: "文章ID (关联 blog_article.article_id)不能为空", trigger: "blur" }
    ],
    nickname: [
      { required: true, message: "评论人昵称 (游客或用户昵称)不能为空", trigger: "blur" }
    ],
    content: [
      { required: true, message: "评论内容不能为空", trigger: "blur" }
    ],
    status: [
      { required: true, message: "状态 (0=待审核, 1=已显示)不能为空", trigger: "change" }
    ],
  }
})
